...
class SL_BEEHIVE:
    ...
    def inject(self, packet, channel=None, count=1, delay=0, page=0):
        ...
        if len(packet) > maxp:
            ...
            tosend = len(packet)
        self.__send_cmd("setTxPayload", "00 %02x%s" % ((len(packet)), binascii.hexlify(packet[:tosend])))
        if len(packet) > maxp:
            self.__send_cmd("setTxPayload", "%d %s" % (tosend + 1, binascii.hexlify(packet[tosend:])))
        #print('sending', len(packet), 'bytes:', binascii.hexlify(packet))
